...
import { FollowersShowService } from "../../src/api/services/follows/FollowersShowService";
...
let followersShowService: FollowersShowService;
...
describe("Services :: Follows :: Follow Creation", () => {
  beforeEach(() => {
    ...
    followersShowService = new FollowersShowService(usersRepository);
  });

  it("should be able to show a user followers", async () => {
    ...
    const followersShow = await followersShowService.execute({
      cuid: `${userCreated.cuid}`,
    });
    expect(followersShow.length).toBe(1);
  });
